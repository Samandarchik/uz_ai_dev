...
class _QtyDialogState extends State<_QtyDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final batches = widget.product.batchesFor(_qty);
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          TextField(
            ...
            onChanged: (v) =>
                setState(() => _qty = int.tryParse(v.trim()) ?? 0),
            onSubmitted: (_) => Navigator.pop(context, _qty),
          ),
          const SizedBox(height: 10),
          Text(
            _qty > 0
                ? '$_qty dona → $batches partiya '
                    '(${batches * widget.product.batchQty} talik masalliq)'
                : 'Partiya: ${widget.product.batchQty} dona',
            style: TextStyle(
              fontSize: 13,
              color: _qty > 0 ? Colors.brown.shade700 : Colors.grey.shade600,
              fontWeight: FontWeight.w500,
            ),
          ),
        ],
