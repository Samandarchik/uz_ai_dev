...
| `lib/shef/` | `shef`: ishlab chiqarish buyurtmasi yaratish, bosqichlarni qabul/rad, полуфабрикат limiti va qoldig'i, **тех карта tahriri (narxsiz)** | `ShefHomeUi`, `ShefCreateOrderUi`, `ShefOrderDetailUi`, `PfStockPage` (пф qoldig'i: Bor/Band/Mumkin), `ShefTechCardCategoriesPage`/`ShefTechCardProductsPage` (`shef_tech_card_page.dart` — kategoriya → mahsulot → `TechCardEditorPage(canEditPrices: false)`). Provider: `ShefProvider` + admin manbalari (`ProductProviderAdmin`, `CategoryProviderAdmin`) |
...
| ui | `lib/admin/ui/tech_card_editor_page.dart` | `TechCardEditorPage` — Excel «тех карта» varag'iga o'xshash muharrir. `canEditPrices: false` (shef rejimi) — «Цена продажи»/«Прибыль»/«Доп. расходы» faqat o'qiladi, masalliqning qo'lda narx bloki chiqmaydi |
...
| **Shef тех картани tahrirlaydi** | `lib/shef/ui/shef_tech_card_page.dart` (kategoriyalar → mahsulotlar; kirish nuqtasi `shef/ui/shef_home_ui.dart` AppBar «Тех карта» ikonasi) → `admin/ui/tech_card_editor_page.dart` `canEditPrices: false` bilan. Ruxsat: shefning `category_list` i (admin `user_edit_dialog.dart` dagi «Тех карта kategoriyalari» chiplari); `GET /api/categories` ni backend shef uchun O'ZI filtrlaydi — Flutter'da qo'shimcha filtr YO'Q. Narxni ko'rish/tahrir: `production/ui/widgets/price_history_sheet.dart` → `allowManualEdit: false` bo'lsa qo'lda narx bloki chizilmaydi |
